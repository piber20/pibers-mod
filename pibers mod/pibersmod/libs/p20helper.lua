...
local currentVersion = 121

--remove any previous versions that may exist

local p20OldDefaultModSave = nil
local p20OldModSave = nil
local p20OldOldSave = nil

if p20 then

	if p20.DefaultModSave then
		p20OldDefaultModSave = p20.CopyTable(p20.DefaultModSave)
	end
	if p20.ModSave then
		p20OldModSave = p20.CopyTable(p20.ModSave)
	end
	if p20.OldSave then
		p20OldOldSave = p20.CopyTable(p20.OldSave)
	end
...
if not p20 then
	...
	p20.AddCustomCallback(p20, ModCallbacks.MC_USE_PILL, function(_, pillEffect)
		...
		if player then
			...
			local pillColor = nil
			for i=0, 200 do
				if itemPool:GetPillEffect(i) == pillEffect then
					pillColor = i
					break
				end
			end
	...
	p20.AddCustomCallback(p20, p20Callbacks.MC_POST_FIRST_GAME_START, function()
		...
		for i=1, 10000 do
			if not itemConfig:GetCollectible(i) then
		...
		for i=1, 10000 do
			if not itemConfig:GetTrinket(i) then
		...
		for i=1, 10000 do
			if not itemConfig:GetCard(i) then
		...
		for i=1, 10000 do
			if not itemConfig:GetPillEffect(i) then
	...
	p20.DefaultModSave = {
		...
		BatteriesPickedUp = 0,
		ModConfigSave = false,
		MinimapAPISave = false
	}
	p20.ModSave = p20.CopyTable(p20.DefaultModSave)
	p20.OldSave = p20.CopyTable(p20.DefaultModSave)
	
	if p20OldDefaultModSave then
		p20.DefaultModSave = p20.FillTable(p20.DefaultModSave, p20OldDefaultModSave)
	end
	if p20OldModSave then
		p20.ModSave = p20.FillTable(p20.ModSave, p20OldModSave)
	end
	if p20OldOldSave then
		p20.OldSave = p20.FillTable(p20.OldSave, p20OldOldSave)
	end
	...
	p20MinimapAPIBuildsToSave = p20MinimapAPIBuildsToSave or {}
	function p20.AddSaveableMinimapAPIBuild(build)
		if build then
			p20MinimapAPIBuildsToSave[build] = true
		end
	end
	...
	function p20.Save(bypassGameStart, menuExit)
		if p20.GameStarted or bypassGameStart then
			p20.ModSave.Config.HudOffset = p20.HudOffset
			
			if ModConfigMenu then
			
				p20.ModSave.Config.HudOffset = ModConfigMenu.Config["General"].HudOffset
				p20.ModSave.Config.Overlays = ModConfigMenu.Config["General"].Overlays
				p20.ModSave.Config.ChargeBars = ModConfigMenu.Config["General"].ChargeBars
				p20.ModSave.Config.BigBooks = ModConfigMenu.Config["General"].BigBooks
			
			end
		
			if ScreenHelper then
			
				p20.HudOffset = ScreenHelper.GetOffset()
				p20.ModSave.Config.HudOffset = ScreenHelper.GetOffset()
	...
	p20.AddCustomCallback(p20, ModCallbacks.MC_PRE_GAME_EXIT, function(_, menuExit)
		p20.GameStarted = false
		p20.IsSaveGame = false
		p20.Save(true, menuExit)
		p20.ClearRunSave(true)
	end)
	
	--hook into mod config menu
	p20.AddCustomCallback(p20, p20Callbacks.MC_POST_FIRST_GAME_START, function()
	...
	function p20.GetCurrentItemPool()
		...
		local itemPool = game:GetItemPool()
		local itemPoolType = itemPool:GetPoolForRoom(roomType, getCurrentItemPoolRNG:Next())
	...
	function p20.GetRandomCollectible(pool, decrease, seed)
		...
		local collectible = itemPool:GetCollectible(pool, decrease, seed)
	...
	function p20.GetRandomCard(allowPlayingCards, allowRunes, onlyRunes, seed)
		...
		local card = itemPool:GetCard(seed, allowPlayingCards, allowRunes, onlyRunes)
	...
	function p20.AddCardToTables(isCardTable, cardPoolTable, cardID, toPool)
		if cardID > 0 then
			...
			if game:IsGreedMode() and not itemConfig:GetCard(cardID).GreedModeAllowed then
				toPool = false
			end
	...
	p20.AddCustomCallback(p20, ModCallbacks.MC_INPUT_ACTION, function(_, entity, inputHook, buttonAction)
		if entity and entity:ToPlayer() then
			...
			if inputHook == InputHook.IS_ACTION_TRIGGERED and forcingActionTriggered[controllerIndex] and forcingActionTriggered[controllerIndex][buttonAction] ~= nil then
	...
	function p20.GetPlayerTakeFullDamage(player)
		...
		return (data.TakeFullDamage or level:GetStage() > 6)
		...
		if p20PlayerFormData[playerForm] then
			...
			for _, value in pairs(p20PlayerFormData[playerForm].Items) do
				if type(value) == "number" and not p20.PlayerHadCollectible(player, value) then
					...
					if itemConfig:GetCollectible(value).Type == ItemType.ITEM_ACTIVE then
						wasActiveAdded = true
					end
				end
		...
		if #pickupToRestock >= 1 then
			...
			for i=1, #pickupToRestock do
				...
				if pickupToRestock[i] then
					...
					if pickupToRestock[i].RoomSeed == room:GetDecorationSeed() then
					
						if Isaac.GetFrameCount() >= pickupToRestock[i].Frame + 60 then
						
							local position = pickupToRestock[i].Position
							
							local respawnedPickup = game:Spawn(EntityType.ENTITY_PICKUP, pickupToRestock[i].Variant or 0, position, p20.VECTOR_ZERO, nil, pickupToRestock[i].SubType or 0, p20.GetRNGNext()):ToPickup()
							
							if pickupToRestock[i].DoPoof then
								game:Spawn(EntityType.ENTITY_EFFECT, EffectVariant.POOF01, position, p20.VECTOR_ZERO, nil, 0, p20.GetRNGNext())
							end
	...
	function p20.ShouldPickupPlayAppearAnimation(pickup)
		...
		if pickup and pickup:Exists() then
			...
			if (room:GetFrameCount() <= 0 and not room:IsFirstVisit()) or pickup:IsShopItem() then
				return false
			end
	...
	function p20.GetClampedToTilePosition(pos)
		...
		local posClamped = room:GetGridPosition(gridIndex)
			...
			for j=1, #costumeTable do
				...
				and costumeTable[j].Type then
						...
						or (currentValueIsNumber and currentValueToCheck > 0) then
							for i, costume in pairs(costumeTable[j].Costume) do
								if costumeTable[j].Type == ItemType.ITEM_PASSIVE then
									player:AddCostume(itemConfig:GetCollectible(costume), false)
								elseif costumeTable[j].Type == ItemType.ITEM_TRINKET then
									player:AddCostume(itemConfig:GetTrinket(costume), false)
								else
					...
					or (currentValueIsNumber and dataCostumeTable[j] > 0 and currentValueToCheck <= 0) then
						for i, costume in pairs(costumeTable[j].Costume) do
							if costumeTable[j].Type == ItemType.ITEM_PASSIVE then
								player:RemoveCostume(itemConfig:GetCollectible(costume))
							elseif costumeTable[j].Type == ItemType.ITEM_TRINKET then
								player:RemoveCostume(itemConfig:GetTrinket(costume))
							else
	...
	function p20.DoBigbookPause()
		...
		if p20.ReimplementedBigbook then
			...
			sfx:Stop(SoundEffect.SOUND_BERKANO)
			player:AnimateSad()
				...
				if toReturn ~= true then
					if p20data.PlayerCollect then
						...
						if p20data.ActAsBlendedHeart then
							...
							if boneHeartsCanAdd > 0 then
								...
								if p20data.PlayAllHeartSounds then
									sfx:Play(soundToPlay, 1, 0, false, 1.0)
								end
							end
							if heartsCanAdd > 0 then
							...
							if p20data.AddBlackHearts and p20data.AddBlackHearts > 0 then
								...
								if p20data.PlayAllHeartSounds then
									sfx:Play(SoundEffect.SOUND_UNHOLY, 1, 0, false, 1.0)
								end
							end
						end
							...
							if p20data.CanDoPennyTrinketEffects then
								...
								if player:HasTrinket(TrinketType.TRINKET_BLOODY_PENNY) then
									if p20.GetRandomNumber(1, 2, pickupRNG) == 1 then
										Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_HEART, HeartSubType.HEART_HALF, room:FindFreePickupSpawnPosition(pickup.Position, 0, true), p20.VECTOR_ZERO, pickup)
									end
								end
								
								if player:HasTrinket(TrinketType.TRINKET_BURNT_PENNY) then
									if p20.GetRandomNumber(1, 2, pickupRNG) == 1 then
										Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_BOMB, BombSubType.BOMB_NORMAL, room:FindFreePickupSpawnPosition(pickup.Position, 0, true), p20.VECTOR_ZERO, pickup)
									end
								end
								
								if player:HasTrinket(TrinketType.TRINKET_FLAT_PENNY) then
									if p20.GetRandomNumber(1, 2, pickupRNG) == 1 then
										Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_KEY, KeySubType.KEY_NORMAL, room:FindFreePickupSpawnPosition(pickup.Position, 0, true), p20.VECTOR_ZERO, pickup)
									end
								end
						...
						if player:NeedsCharge() then
							...
							local itemConfig = Isaac.GetItemConfig()
							local maxCharge = itemConfig:GetCollectible(player:GetActiveItem()).MaxCharges
							...
							if p20data.FullCharge and not p20data.DoNotFullCharge12Charge or (p20data.FullCharge and p20data.DoNotFullCharge12Charge and maxCharge <= 6) then
								...
								if chargeToAdd > 0 then
									...
									if p20data.OverCharge then
										sfx:Play(SoundEffect.SOUND_THUMBSUP, 0.5, 0, false, 1.0)
									end
								end
							end
						end
					...
					if p20data.CollectSound then
						sfx:Play(p20data.CollectSound, 1, 0, false, 1.0)
					end
	...
	p20.AddCustomCallback(p20, ModCallbacks.MC_PRE_PICKUP_COLLISION, function(_, pickup, collider, low)
		...
		if collider and collider.Type == EntityType.ENTITY_PLAYER and collider:ToPlayer() then
			...
			elseif not sprite:IsPlaying("Collect") and pickup:Exists() then
				...
				local game = Game()
				local level = game:GetLevel()
				local originalRoomIndex = level:GetCurrentRoomIndex()
				
				p20.DelayFunction(function(pickup, player)
				
					local game = Game()
					local level = game:GetLevel()
					local newRoomIndex = level:GetCurrentRoomIndex()
				
					if player:Exists()
					and originalRoomIndex == newRoomIndex
					and (not pickup:Exists() or pickup:GetSprite():IsPlaying("Collect")) then
		...
		if p20PickupData[variant] and p20PickupData[variant][subType] then
			...
			if sprite:IsEventTriggered("DropSound") then
				if p20data.DropSound then
					local sfx = SFXManager()
					sfx:Play(p20data.DropSound, 1, 0, false, 1.0)
				end
	...
	p20.AddCustomCallback(p20, ModCallbacks.MC_POST_NEW_ROOM, function()
		...
		for _, player in pairs(p20.GetPlayers()) do
			...
			if player:HasCollectible(CollectibleType.COLLECTIBLE_BLANKET) and room:GetType() == RoomType.ROOM_BOSS then
